...
def summarize():
    ...
    if request.method == 'POST':
        ...
        print(keyword)
        tweetText = getTweetsText(keyword,25)
        if tweetText: 
...
def offline() :
    if request.method == 'POST' :
        ...
        return render_template('uploadSummary.html',titel = "Your summary is here",summary = summary)
...
def tweetSum(trend):
    ...
    print(trend)
    tweetText = getTweetsText(trend,25)
    if tweetText: 
        summary = getSummary(tweetText).upper()
        return render_template('uploadSummary.html', title="What Twitter says about "+trend,summary = summary )
    else:
        return "No Tweets :("
